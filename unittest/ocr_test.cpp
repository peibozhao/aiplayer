...
TEST_CASE("dbnet", "ocr") {
    ...
    float ratio = 1.f;
    ratio = float(maxside_len) / float(image.rows > image.cols ? image.rows : image.cols);
    ...
    REQUIRE_FALSE(image.empty());
    DbNet dbnet;
    REQUIRE(dbnet.InitModel("dbnet.mnn"));
    std::vector<Box> boxes = dbnet.Detect(image);
    for (int i = 0; i < boxes.size(); ++i) {
        const Box &box = boxes[i];
        cv::Rect rect(box.x, box.y, box.width, box.height);
        cv::rectangle(image, rect, cv::Scalar(255, 0, 0),1);
        cv::putText(image, std::to_string(i), cv::Point(box.x, box.y), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255), 2);
        SPDLOG_INFO("{}: {}, {}, {}, {}", i, rect.x, rect.y, rect.width, rect.height);
    }
...
TEST_CASE("crnn", "ocr") {
    ...
    crnn_config.hot_word = {"切", "换", "立", "刻", "前", "往", "继", "续", "点", "关", "闭", "锁", "分", "享", "检", "视", "确", "定", "出", "击", "撤", "退"};
